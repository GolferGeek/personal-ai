...
// https://mui.com/material-ui/guides/nextjs-app-router/#emotion
export default function NextAppDirEmotionCacheProvider(props: NextAppDirEmotionCacheProviderProps) {
  ...
  useServerInsertedHTML(() => {
    ...
    let styles = '';
    const cacheKey: string = registry.cache.key ?? 'emotion';
    let dataEmotionAttribute = cacheKey;
    ...
    return (
      <React.Fragment>
        {globals.map(({ name, style }) => (
          <style
            key={name}
            data-emotion={`${cacheKey}-global ${name}`}
